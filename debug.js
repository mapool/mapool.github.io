...
function debug() {
    ...
    return isNaN(port) || (options || (options = {}), options.port = port), port > 0 && !options.port && (options.port = port || 8e3), options.https ? F.https("debug", options) : (F.http("debug", options), void(first ? F.emit("debug-start") : F.emit("debug-restart")))
}

function app() {
    function onFilter(path, isDirectory) {
        return !isDirectory && REG_THEMES.test(path) ? REG_THEMES_INDEX.test(path) : !!isDirectory || REG_EXTENSION.test(path)
    }
    ...
    function onComplete(f) {
        fs.readdir(directory, function (err, arr) {
            for (var length = arr.length, i = 0; i < length; i++) {
                var name = arr[i];
                "debug.js" !== name && REG_FILES.test(name) && f.push(name)
            }
            length = f.length;
            for (var i = 0; i < length; i++) {
                var name = f[i];
                void 0 === files[name] && (files[name] = isLoaded ? 0 : null)
            }
            refresh()
        })
    }
    ...
    function refresh() {
        for (var filenames = Object.keys(files), length = filenames.length, i = 0; i < length; i++) {
            var filename = filenames[i];
            ! function (filename) {
                async.await(function (next) {
                    fs.stat(filename, function (err, stat) {
                        var stamp = "--- # --- [ " + (new Date).format("yyyy-MM-dd HH:mm:ss") + " ] ";
                        if (err) delete files[filename], changes.push(stamp.replace("#", "REM") + prefix + filename.replace(directory, "")), force = !0;
                        else {
                            var ticks = stat.mtime.getTime();
                            null != files[filename] && files[filename] !== ticks && (changes.push(stamp.replace("#", 0 === files[filename] ? "ADD" : "UPD") + prefix + filename.replace(directory, "")), force = !0), files[filename] = ticks
                        }
                        next()
                    })
                })
        ...
        async.complete(function () {
            if (isLoaded = !0, setTimeout(refresh_directory, speed), onIncrease(), 1 === status && force) {
                ...
                for (var length = changes.length, i = 0; i < length; i++) console.log(changes[i]);
                changes = [], force = !1
            }
        })
    }
    ...
    process.on("SIGTERM", end), process.on("SIGINT", end), process.on("exit", end), process.pid > 0 && (console.log(prefix + "PID: " + process.pid + " (v" + VERSION + ")"), pid = path.join(directory, "debug.pid"), fs.writeFileSync(pid, process.pid), pidInterval = setInterval(function () {
        fs.exists(pid, function (e) {
            e || (fs.unlink(pid, noop), null !== app && (isSkip = !0, process.kill(app.pid)), process.exit(0))
        })
...
const isDebugging = process.argv.indexOf("debugging") !== -1,
    ...
    VERSION = "6.0",
    TIME = 2e3,
    REG_FILES = /config\-debug|config\-release|config|versions|sitemap|dependencies|\.js|\.resource/i,
    REG_THEMES = /\/themes\//i,
    REG_THEMES_INDEX = /themes(\/|\\)?[a-z0-9_.-]+(\/|\\)?index\.js/i,
    REG_EXTENSION = /\.(js|resource|package)/i;
